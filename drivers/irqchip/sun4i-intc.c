	...
	while ((irq = mmio_read32(irqdev->address + INTC_VECTOR_REG) >> 2)) {
		...
		if (likely(irq_vectors[irq].handler)) {
			...
			if (irq_vectors[irq].dev)
				warn("IRQ %d last registered to device %s", irq,
...
sun4i_intc_probe(struct device *dev)
{
	...
	irqchip_device_register(dev, dev->drvdata);

	return SUCCESS;
}

static int
sun4i_intc_register_irq(struct device *irqdev, struct device *dev, void (*handler)(
				struct device *))
{
	uint8_t irq = dev->irq;
...
struct driver sun4i_intc_driver __driver = {
	.name  = "sun4i-intc",
